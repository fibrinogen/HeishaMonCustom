...
function externalOpenEditor(id){var s=id?externalFind(id):null;externalEditingId=s?s.id:0;document.getElementById('externalEditorTitle').textContent=s?'Edit MQTT sensor':'Add MQTT sensor';document.getElementById('externalName').value=s?s.name:'';document.getElementById('externalTopic').value=s?s.mqttTopic:'';document.getElementById('externalUnit').value=s?s.unit:'';document.getElementById('externalTimeout').value=s?s.staleTimeoutSeconds:120;document.getElementById('externalEnabled').checked=s?!!s.enabled:true;document.getElementById('externalModal').classList.add('open');document.getElementById('externalName').focus();}
function externalCloseEditor(){document.getElementById('externalModal').classList.remove('open');}
function externalSaveEditor(){var name=document.getElementById('externalName').value.trim(),topic=document.getElementById('externalTopic').value.trim(),unit=document.getElementById('externalUnit').value.trim(),timeout=Number(document.getElementById('externalTimeout').value);if(!name||!topic||!Number.isInteger(timeout)||timeout<5||timeout>86400){externalStatus('Please enter valid sensor fields.',true);return;}externalCommand('save',JSON.stringify({id:externalEditingId,enabled:document.getElementById('externalEnabled').checked,name:name,mqttTopic:topic,unit:unit,staleTimeoutSeconds:timeout})).then(externalCloseEditor);}
