...
function wpSync(){
  ...
  var curveAvailable=!!(wpCurveShift&&wpCurveShift.implementation==='curveEndpoints'&&wpCurveShift.writable===true);
  document.querySelectorAll('.wp-curve-row').forEach(function(row){row.style.display=curveAvailable?'flex':'none';});
  var curveNote=document.getElementById('wpCurveUnavailable');if(curveNote)curveNote.style.display=curveAvailable?'none':'block';
  if(curveNote&&!curveAvailable)curveNote.textContent=wpCurveShift&&wpCurveShift.available?'This sensor configuration does not adjust curve endpoints directly.':'Heating-curve endpoint configuration is unavailable for the current heat-pump data.';
  if(curveAvailable){updCell('wpCurveBaseHigh',String(wpCurveShift.baseTargetHigh));updCell('wpCurveBaseLow',String(wpCurveShift.baseTargetLow));updCell('wpCurveEffectiveHigh',String(wpCurveShift.effectiveTargetHigh));updCell('wpCurveEffectiveLow',String(wpCurveShift.effectiveTargetLow));}
...
function wpRefresh(){
  ...
  wpRefreshPromise=fetch('/json',{cache:'no-store'}).then(function(r){if(!r.ok)throw new Error('HTTP '+r.status);return r.json();}).then(function(data){wpItems(data).forEach(function(item){wpValues[item.Topic]=item.Value;updCell(item.Topic+'-Value',String(item.Value));updCell(item.Topic+'-Description',String(item.Description));});return fetch('/wpsettingsconfig',{cache:'no-store'});}).then(function(r){if(!r.ok)throw new Error('HTTP '+r.status);return r.json();}).then(function(data){wpConfig=data;return fetch('/heatingcurveshift',{cache:'no-store'});}).then(function(r){if(!r.ok)throw new Error('HTTP '+r.status);return r.json();}).then(function(data){wpCurveShift=data;wpRefreshPromise=null;wpSync();}).catch(function(e){wpRefreshPromise=null;wpStatus('Update failed: '+e.message,true);});
...
function wpStepCurveBase(which,delta){if(!wpCurveShift||wpCurveShift.implementation!=='curveEndpoints'||wpCurveShift.writable!==true){wpStatus('Heating-curve endpoint configuration is unavailable',true);return;}var key=which==='high'?'baseTargetHigh':'baseTargetLow',current=Number(wpCurveShift[key]);if(!Number.isFinite(current))return;wpCurveShift[key]=Math.round(current+delta);wpCurveShift.effectiveTargetHigh=Number(wpCurveShift.baseTargetHigh)+Number(wpCurveShift.shift);wpCurveShift.effectiveTargetLow=Number(wpCurveShift.baseTargetLow)+Number(wpCurveShift.shift);wpSync();wpQueueStep('curve:'+which,which==='high'?'SetZ1HeatCurveBaseHigh':'SetZ1HeatCurveBaseLow',wpCurveShift[key]);}
