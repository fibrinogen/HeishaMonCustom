...
function monitorWebSocket() {
  ...
  websocketMonitorTimer = setInterval(function () {
    if (
      typeof oWebsocket !== "undefined" &&
      Date.now() - lastActivityTime > inactivityTimeout &&
      oWebsocket.readyState === WebSocket.OPEN
    ) {
...
function attemptReconnect() {
  ...
  websocketReconnectTimer = setTimeout(function () {
    ...
    if (!websocketShuttingDown && !bConnected) {
...
function startWebsockets() {
  ...
  if (
    typeof oWebsocket !== "undefined" &&
    (oWebsocket.readyState === WebSocket.OPEN ||
      oWebsocket.readyState === WebSocket.CONNECTING)
  )
    return;
      ...
      if (evt.data.startsWith("{")) {
        ...
        if (j.logMsg != null) {
          ...
          if (j.data.stats) {
            ...
            updCell(
              j.data.heishavalues.topic + "-Description",
              j.data.heishavalues.description,
            );
          } else if (j.data.dallasvalues) {
            var dID = j.data.dallasvalues.sensorID;
            if (j.data.dallasvalues.value !== undefined)
              updCell(
                "SensorID-" + dID + "-Temperature",
                j.data.dallasvalues.value,
              );
            if (j.data.dallasvalues.present !== undefined)
              updDallasPresence(dID, j.data.dallasvalues.present);
          } else if (j.data.s0values) {
            updCell(
              "s0port-" + j.data.s0values.s0port + "-Watt",
              j.data.s0values.Watt,
            );
            updCell(
              "s0port-" + j.data.s0values.s0port + "-Watthour",
              j.data.s0values.Watthour,
            );
            updCell(
              "s0port-" + j.data.s0values.s0port + "-WatthourTotal",
              j.data.s0values.WatthourTotal,
            );
...
function closeWebsocketForNavigation() {
  ...
  bConnected = false;
  if (websocketMonitorTimer) {
    clearInterval(websocketMonitorTimer);
    websocketMonitorTimer = null;
  }
...
function updCell(id, val) {
  if (
    typeof dashboardDisplayValue === "function" &&
    id.indexOf("TOP") === 0 &&
    id.slice(-6) === "-Value"
  ) {
...
setInterval(function () {
  document
    .querySelectorAll(".offline-duration[data-offline-since]")
