...
var dashboardTemperatureTopics = {
  TOP5: 1,
  TOP6: 1,
  TOP7: 1,
  TOP9: 1,
...
function dashboardDisplayValue(topic, value) {
  if (dashboardTemperatureTopics[topic]) {
    ...
    if (!Number.isFinite(numeric) || numeric < -60 || numeric > 150)
      return "N/A";
  }
  return String(value);
}
function renderDashboardHeatRequest() {
  ...
  if (!dashboardSemantic || dashboardSemantic.semantic === "heatCurveShift") {
  ...
  var rawValid =
    dashboardSemantic.rawValidForSemantic === true && Number.isFinite(raw);
  var semanticKnown =
    dashboardSemantic.semanticKnown === true ||
    (!!dashboardSemantic.semantic && dashboardSemantic.semantic !== "unknown");
  var editable = semanticKnown && dashboardSemantic.writable === true;
  value.textContent = rawValid
    ? String(raw)
    : Number.isFinite(raw)
      ? semanticKnown
        ? "Invalid (raw: " + String(raw) + ")"
  ...
  if (recover) {
    recover.textContent =
      dashboardSemantic.semantic === "heatCurveShift" ? "Set 0" : "Set valid";
    recover.style.display = editable && !rawValid ? "inline-flex" : "none";
  }
}
function renderDashboardCurveShift() {
  ...
  if (value)
    value.textContent =
      available && dashboardCurveShift.valueValid !== false
        ? String(dashboardCurveShift.shift)
        : available && dashboardCurveShift.rawValue !== null
          ? "Invalid (raw: " + String(dashboardCurveShift.rawValue) + ")"
          : "N/A";
  if (control)
    control.querySelectorAll("button").forEach(function (button) {
...
function renderDashboard(data) {
  ...
  items.forEach(function (item) {
    updCell(
      item.Topic + "-Value",
      dashboardDisplayValue(item.Topic, item.Value),
    );
...
function setDashboardSelectValue(id, value) {
  ...
  if (select && document.activeElement !== select && !isNaN(Number(value)))
...
function syncDashboardControls() {
  ...
  var operationMode = Number(dashboardValues.TOP4);
  if (operationMode === 7) operationMode = 2;
  if (operationMode === 8) operationMode = 6;
  ...
  if (cancelSterilization)
    cancelSterilization.disabled = dashboardWorkflow.type !== "sterilization";
...
function sendDashboardCommand(command, value) {
  if (dashboardCommandBusy) {
    setDashboardStatus("Please wait for the current command to finish", false);
...
function setDashboardToggle(toggle, command) {
  ...
  sendDashboardCommand(command, toggle.checked ? 1 : 0)
    .catch(function () {
...
function setDashboardSelect(select, command) {
  ...
  sendDashboardCommand(command, select.value).then(
    function () {
...
function queueDashboardStep(command, topic, value) {
  if (dashboardStepTimers[topic])
    window.clearTimeout(dashboardStepTimers[topic]);
...
function stepZone1Heat(delta) {
  if (dashboardSemantic && dashboardSemantic.semantic === "heatCurveShift") {
    stepHeatingCurveShift(delta);
    return;
  }
  if (
    !dashboardSemantic ||
    dashboardSemantic.writable !== true ||
    dashboardSemantic.semanticKnown !== true
  ) {
    setDashboardStatus(
      "Zone 1 request is not writable for the current configuration",
      true,
    );
    return;
  }
  ...
  var rawValid =
    dashboardSemantic.rawValidForSemantic === true && Number.isFinite(current);
  if (!rawValid)
    current = dashboardSemantic.semantic === "heatCurveShift" ? 0 : min;
  ...
  var next = Math.max(min, Math.min(max, Math.round(current + delta)));
  var command =
    dashboardSemantic.semantic === "heatCurveShift"
...
function stepHeatingCurveShift(delta) {
  if (
    !dashboardCurveShift ||
    dashboardCurveShift.available !== true ||
    dashboardCurveShift.writable !== true
  ) {
    setDashboardStatus(
      "Heating curve shift is not writable for the current configuration",
      true,
    );
    return;
  }
  var current = Number(dashboardCurveShift.shift),
    min = Number(dashboardCurveShift.min),
    max = Number(dashboardCurveShift.max);
  if (!Number.isFinite(current)) current = 0;
  var next = Math.max(min, Math.min(max, Math.round(current + delta)));
  dashboardCurveShift.shift = next;
  dashboardCurveShift.valueValid = true;
  renderDashboardCurveShift();
...
function recoverZone1Heat() {
  if (
    ...
    return;
  var min = Number(dashboardSemantic.min),
    max = Number(dashboardSemantic.max);
